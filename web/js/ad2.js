(function(window) {
    ...
    var create = function() {
    	...
    	return function (madservevariables, adContainer) {
    		...
    		if (!madservevariables.s) return; // no Placement Integration ID

    		var src = madservevariables.requesturl;
    		// not sure what the .service could be -- haven't seen this before
            ...
            src += "?" + cgi.join('&'); // update src url

            // create the HTML containter dev element
            ...
            var varsloaded = function() {
            	var ads = window[id]; // when would this ever produce more than one ad? ids SHOULD be unique

            	for (iter=0; iter<ads.length; ++iter) {
            		var ad = ads[iter];
            		// not sure what the .target is -- haven't seen this before
            		...
            		// if no errors, determine type of ad?
            		if (ad.img) { // haven't seen this format yet...
            		...
            		} else if (ad.content) { // this is the case for our banner ads
            			container.innerHTML = '';
